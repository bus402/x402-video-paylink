...
function App() {
  const [status, setStatus] = useState<"loading" | "paywall" | "playing">("loading");
  const [jwt, setJwt] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    // Extract stream ID from streamUrl
    const streamId = window.x402?.streamUrl?.match(/\/stream\/([^\/\.]+)/)?.[1];
  ...
  if (status === "paywall" && window.x402) {
    return (
      <Providers>
        <PaywallApp
          streamUrl={window.x402.streamUrl || ""}
  ...
  if (status === "playing" && jwt && window.x402?.streamUrl) {
    return <VideoPlayer streamUrl={window.x402.streamUrl} jwt={jwt} />;
  }
