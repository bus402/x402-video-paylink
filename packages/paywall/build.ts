...
const OUTPUT_TS = path.join("gen", "template.ts");

const options: esbuild.BuildOptions = {
  entryPoints: ["index.tsx", "styles.css"],
  bundle: true,
  metafile: true,
  outdir: DIST_DIR,
  treeShaking: true,
...
`;
      ...
      // Also write template.js for runtime use in Docker
      const jsContent = `// THIS FILE IS AUTO-GENERATED - DO NOT EDIT
/**
 * The pre-built, self-contained paywall template with inlined CSS and JS
 */
exports.PAYWALL_TEMPLATE = ${JSON.stringify(html)};
`;
      const outputJs = path.join("gen", "template.js");
      fs.writeFileSync(outputJs, jsContent);
      console.log(`Generated template.js with bundled HTML (${html.length} bytes)`);
    } else {
